...
def process_car_rental_data(data_date):
    ...
    spark = SparkSession.builder \
        ...
        .config("spark.jars", "gs://snowflake_projectss/snowflake_jars/spark-snowflake_2.12-2.15.0-spark_3.4.jar,gs://snowflake_projectss/snowflake_jars/snowflake-jdbc-3.16.0.jar") \
        .getOrCreate()

    # Define GCS file path based on the date argument
    gcs_file_path = f"gs://snowflake_projectss/car_rental_data/car_rental_daily_data/car_rental_{data_date}.json"
    ...
    snowflake_options = {
        "sfURL": "https://n****c-f****6.snowflakecomputing.com",
        "sfAccount": "n****c-f****6",
        "sfUser": "username",
        "sfPassword": "password",
        "sfDatabase": "datebase_name",
        "sfSchema": "PUBLIC",
    ...
    fact_df = fact_df.alias("fact") \
        ...
        .filter(col("cust.is_current")== True)\
